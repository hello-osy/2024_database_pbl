...
import { getSAVYardRoutes } from '../../../router';

export default {
  data() {
    return {
      yards: [], // SAV Yards 데이터를 저장
    };
  },
  methods: {
    addYardToPage() {
    ...
    async updateYards() {
      await updateYardsToServer("SAV"); // Fixed: Changed LA to SAV
    },
    updateYardsList() {
      this.yards = getSAVYardRoutes();
      console.log("yard 추가 후 경로: ", this.$router.options.routes);
    },
    viewYards() {
      console.log("현재 SAV Yards 데이터:", this.yards); // Fixed: Changed LA to SAV
      alert(`SAV Yards 개수: ${this.yards.length}`); // Fixed: Changed LA to SAV
    },
  },
  created() {
    this.updateYardsList();
